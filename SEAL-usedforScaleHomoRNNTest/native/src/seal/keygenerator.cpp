...
namespace seal
{
    KeyGenerator::KeyGenerator(const SEALContext &context, size_t partyNum) : context_(context)
    {
        // Verify parameters
    ...
    void KeyGenerator::skadd(SecretKey &sk)
    {
        ...
        RNSIter secret_key_array(secret_key_.data().data(), coeff_count);
        RNSIter seckey_add_array(sk.data().data(), coeff_count);
        SEAL_ITERATE(iter(secret_key_array, seckey_add_array, coeff_modulus), coeff_modulus_size, [&](auto I) {
            add_poly_coeffmod(get<0>(I), get<1>(I), coeff_count, get<2>(I), get<0>(I));
        });
        set_poly(secret_key_.data().data(), coeff_count, coeff_modulus_size, secret_key_array_.get());
    }
    ...
    PublicKey KeyGenerator::generate_pk(bool save_seed) const
    {
        ...
        encrypt_zero_symmetric_keygen(secret_key_, context_, context_data.parms_id(), true, save_seed, public_key.data(), party_num_);
    ...
    void KeyGenerator::generate_one_kswitch_key(ConstRNSIter new_key, vector<PublicKey> &destination, bool save_seed)
    {
        ...
        SEAL_ITERATE(iter(new_key, key_modulus, destination, size_t(0)), decomp_mod_count, [&](auto I) {
            ...
            encrypt_zero_symmetric_keygen(
                secret_key_, context_, key_context_data.parms_id(), true, save_seed, get<2>(I).data(), party_num_);
