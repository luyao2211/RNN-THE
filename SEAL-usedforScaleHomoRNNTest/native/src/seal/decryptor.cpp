...
namespace seal
{
    ...
    void Decryptor::partial_decrypt(const Ciphertext &encrypted, Plaintext &noise, Plaintext &destination)
    {
        // Verify that encrypted is valid.
        if (!is_valid_for(encrypted, context_))
        {
        ...
        if (encrypted.parms_id() != noise.parms_id() || encrypted.scale() != noise.scale())
        {
            throw invalid_argument("incorrect noise");
        }
        ...
        if (parms.scheme() == scheme_type::ckks)
        {
            ...
            ConstRNSIter noise_array(noise.data(), coeff_count);
            RNSIter destination_array(destination.data(), coeff_count);

            SEAL_ITERATE(iter(c1, secret_key_array, noise_array, coeff_modulus, destination_array), coeff_modulus_size, [&](auto I)
            {
                dyadic_product_coeffmod(get<0>(I), get<1>(I), coeff_count, get<3>(I), get<4>(I));
                add_poly_coeffmod(get<4>(I), get<2>(I), coeff_count, get<3>(I), get<4>(I));
            });
    ...
    void Decryptor::final_decrypt(const Ciphertext &encrypted, Plaintext *c1s, size_t partyNum, Plaintext &destination)
    {
        // Verify that encrypted is valid.
        if (!is_valid_for(encrypted, context_))
        {
        ...
        {
            if (encrypted.parms_id() != c1s[i].parms_id() || encrypted.scale() != c1s[i].scale())
            {
                throw invalid_argument("incorrect c1s");
            }
        }
        ...
        if (parms.scheme() == scheme_type::ckks)
        {
            ...
            SEAL_ITERATE(iter(c0, c1, secret_key_array, coeff_modulus, destination_array), coeff_modulus_size, [&](auto I) {
            ...
            for (size_t i = 0; i < partyNum; i++)
            {
                ConstRNSIter c1s_array(c1s[i].data(), coeff_count);
                SEAL_ITERATE(iter(c1s_array, coeff_modulus, destination_array), coeff_modulus_size, [&](auto I) {
