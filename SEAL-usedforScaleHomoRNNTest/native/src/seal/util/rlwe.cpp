...
namespace seal
{
    namespace util
    {
        ...
        void encrypt_zero_symmetric_keygen(
            const SecretKey &secret_key, const SEALContext &context, parms_id_type parms_id, bool is_ntt_form,
            bool save_seed, Ciphertext &destination, size_t partyNum)
        {
#ifdef SEAL_DEBUG
            if (!is_valid_for(secret_key, context))
            {
            ...
            // Sample e <-- chi
            Pointer<unsigned __int64, void> *noise = new Pointer<unsigned __int64, void>[partyNum];
            for (size_t i = 0; i < partyNum; i++)
            {
                noise[i] = allocate_poly(coeff_count, coeff_modulus_size, pool);
                SEAL_NOISE_SAMPLER(bootstrap_prng, parms, noise[i].get());
            }
            ...
            for (size_t i = 0; i < coeff_modulus_size; i++)
            {
                ...
                {
                    // Transform the noise e into NTT representation
                    for (size_t j = 0; j < partyNum; j++)
                    {
                        ntt_negacyclic_harvey(noise[j].get() + i * coeff_count, ntt_tables[i]);
                    }
                }
                else
                {
                ...
                if (type == scheme_type::bgv)
                {
                    ...
                    {
                        multiply_poly_scalar_coeffmod(
                            noise[j].get() + i * coeff_count, coeff_count, plain_modulus.value(), coeff_modulus[i],
                            noise[j].get() + i * coeff_count);
                    }
                }
                ...
                {
                    add_poly_coeffmod(
                        noise[j].get() + i * coeff_count, c0 + i * coeff_count, coeff_count, coeff_modulus[i],
                        c0 + i * coeff_count);
                }
                // (as + noise, a) -> (-(as + noise), a),
                negate_poly_coeffmod(c0 + i * coeff_count, coeff_count, coeff_modulus[i], c0 + i * coeff_count);
